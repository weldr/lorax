...
livemedia-creator first uses a bootable ISO image containing Anaconda to start a
virtual machine with qemu. Then, the virtual machine runs Anaconda with a Kickstart file
you provide to install a Fedora system into a filesystem image. Last, the filesystem image
is used by lorax to create the bootable media.
...
Fedora installation in a filesystem image. The ``%post`` section of the Kickstart
is used to customize the installed system. The filesystem image is created from
scratch by Anaconda within the virtual machine; it is not based upon the ISO image
passed to lmc with the ``--iso`` option.
...
If specified by ``part`` commands in the Kickstart file, the output from the Anaconda
installation can be a partitioned disk image. kpartx can be used to mount and examine
it when there is a problem with the install. It can also be booted using kvm.

To create the final image, the filesystem generated by Anaconda within the
virtual machine is used as the input to lorax. Lorax uses the templates in
/usr/share/lorax/live/ or the live directory below the directory specified by
``--lorax-templates``. The templates are written using the Mako template system
with some extra commands added by lorax.

If you want to stop before creating the final image from the filesystem installed
by Anaconda, use the ``--image-only`` argument with lmc. To skip the install stage and use
a previously created disk image, use ``--disk-image`` or ``--fs-image``.

.. note::
    By default, lmc outputs the final image and the files used to create it
    (kernel, initrd, the squashfs filesystem, etc) to the output directory.
    If you only want the bootable image you can pass ``--iso-only``
    and the other files will be removed. You can also name the ISO by using
    ``--iso-name my-live.iso``.
