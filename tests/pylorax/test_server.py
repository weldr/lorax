...
def _wait_for_status(self, uuid, wait_status, api=0):
    ...
    start = time.time()
    while True:
        resp = self.server.get("/api/v%d/compose/info/%s" % (api, uuid))
...
class ServerAPIV0TestCase(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        self.rawhide = False
        self.maxDiff = None
    ...
    def test_projects_source_00_new_toml_vars(self):
        ...
        resp = self.server.get("/api/v0/projects/source/info/new-repo-2-vars")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        sources = data["sources"]
        print(sources)
...
class ServerAPIV1TestCase(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        self.rawhide = False
        self.maxDiff = None
    ...
    def test_02_blueprints_list(self):
        """Test the /api/v1/blueprints/list route"""
        list_dict = {"blueprints":["example-append", "example-atlas", "example-custom-base", "example-development",
                                   "example-glusterfs", "example-http-server", "example-jboss",
                                   "example-kubernetes"], "limit":20, "offset":0, "total":8}
        resp = self.server.get("/api/v1/blueprints/list")
        data = json.loads(resp.data)
        self.assertEqual(data, list_dict)

        # Make sure limit=0 still returns the correct total
        resp = self.server.get("/api/v1/blueprints/list?limit=0")
        data = json.loads(resp.data)
        self.assertEqual(data["limit"], 0)
        self.assertEqual(data["offset"], 0)
        self.assertEqual(data["total"], list_dict["total"])
    ...
    def test_03_blueprints_info_1(self):
        """Test the /api/v1/blueprints/info route with one blueprint"""
        info_dict_1 = {"changes":[{"changed":False, "name":"example-http-server"}],
                       "errors":[],
                       "blueprints":[{"description":"An example http server with PHP and MySQL support.",
                                   "modules":[HTTP_GLOB,
                                   ...
                                   "groups": [],
                                   "version": "0.0.1"}]}
        resp = self.server.get("/api/v1/blueprints/info/example-http-server")
        data = json.loads(resp.data)
        self.assertEqual(data, info_dict_1)

    def test_03_blueprints_info_2(self):
        """Test the /api/v1/blueprints/info route with 2 blueprints"""
        info_dict_2 = {"changes":[{"changed":False, "name":"example-glusterfs"},
                                  {"changed":False, "name":"example-http-server"}],
                       "errors":[],
                       "blueprints":[{"description": "An example GlusterFS server with samba",
        ...
        resp = self.server.get("/api/v1/blueprints/info/example-http-server,example-glusterfs")
        data = json.loads(resp.data)
        self.assertEqual(data, info_dict_2)

    def test_03_blueprints_info_none(self):
        """Test the /api/v1/blueprints/info route with an unknown blueprint"""
        resp = self.server.get("/api/v1/blueprints/info/missing-blueprint")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertTrue(len(data["errors"]) > 0)
        self.assertEqual(data["errors"][0]["id"], "UnknownBlueprint")
    ...
    def test_04_blueprints_changes(self):
        """Test the /api/v1/blueprints/changes route"""
        resp = self.server.get("/api/v1/blueprints/changes/example-http-server")
        data = json.loads(resp.data)
        ...
        # Make sure limit=0 still returns the correct total
        resp = self.server.get("/api/v1/blueprints/changes/example-http-server?limit=0")
        data = json.loads(resp.data)
        self.assertEqual(data["limit"], 0)
        self.assertEqual(data["offset"], 0)
        self.assertEqual(data["blueprints"][0]["total"], 1)
    ...
    def test_04a_blueprints_diff_empty_ws(self):
        """Test the /api/v1/diff/NEWEST/WORKSPACE with empty workspace"""
        resp = self.server.get("/api/v1/blueprints/diff/example-glusterfs/NEWEST/WORKSPACE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data, {"diff": []})

    def test_05_blueprints_new_json(self):
        """Test the /api/v1/blueprints/new route with json blueprint"""
        test_blueprint = {"description": "An example GlusterFS server with samba",
                       "name":"example-glusterfs",
                       "version": "0.2.0",
                       "modules":[GLUSTERFS_GLOB,
        ...
        resp = self.server.post("/api/v1/blueprints/new",
                                data=json.dumps(test_blueprint),
                                content_type="application/json")
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})

        resp = self.server.get("/api/v1/blueprints/info/example-glusterfs")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        blueprints = data.get("blueprints")
        self.assertEqual(len(blueprints), 1)
    ...
    def test_06_blueprints_new_toml(self):
        """Test the /api/v1/blueprints/new route with toml blueprint"""
        test_blueprint = open(joinpaths(self.examples_path, "example-glusterfs.toml"), "rb").read()
        resp = self.server.post("/api/v1/blueprints/new",
                                data=test_blueprint,
                                content_type="text/x-toml")
    ...
    def test_07_blueprints_ws_json(self):
        """Test the /api/v1/blueprints/workspace route with json blueprint"""
        test_blueprint = {"description": "An example GlusterFS server with samba, ws version",
                       "name":"example-glusterfs",
                       "version": "0.3.0",
                       "modules":[GLUSTERFS_GLOB,
        ...
        resp = self.server.post("/api/v1/blueprints/workspace",
                                data=json.dumps(test_blueprint),
                                content_type="application/json")
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})
    ...
    def test_08_blueprints_ws_toml(self):
        """Test the /api/v1/blueprints/workspace route with toml blueprint"""
        test_blueprint = {"description": "An example GlusterFS server with samba, ws version",
                       "name":"example-glusterfs",
                       "version": "0.4.0",
                       "modules":[GLUSTERFS_GLOB,
    ...
    def test_09_blueprints_ws_delete(self):
        ...
        # Delete it
        resp = self.server.delete("/api/v1/blueprints/workspace/example-glusterfs")
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})
    ...
    def test_10_blueprints_delete(self):
        """Test DELETE /api/v1/blueprints/delete/<blueprint_name>"""
        ...
        # This should delete the git blueprint and the workspace copy
        resp = self.server.delete("/api/v1/blueprints/delete/example-glusterfs")
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})
    ...
    def test_11_blueprints_undo(self):
        """Test POST /api/v1/blueprints/undo/<blueprint_name>/<commit>"""
        resp = self.server.get("/api/v1/blueprints/changes/example-glusterfs")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        ...
        # Revert it to the first commit
        commit = changes[-1]["commit"]
        resp = self.server.post("/api/v1/blueprints/undo/example-glusterfs/%s" % commit)
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})
    ...
    def test_12_blueprints_tag(self):
        """Test POST /api/v1/blueprints/tag/<blueprint_name>"""
        resp = self.server.post("/api/v1/blueprints/tag/example-glusterfs")
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})
    ...
    def test_13_blueprints_diff(self):
        """Test /api/v1/blueprints/diff/<blueprint_name>/<from_commit>/<to_commit>"""
        resp = self.server.get("/api/v1/blueprints/changes/example-glusterfs")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        blueprints = data.get("blueprints")
        ...
        # Get the differences between the two commits
        resp = self.server.get("/api/v1/blueprints/diff/example-glusterfs/%s/%s" % (from_commit, to_commit))
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data, {"diff": [{"new": {"Version": "0.0.1"}, "old": {"Version": "0.2.1"}}]})
    ...
    def test_14_blueprints_depsolve(self):
        """Test /api/v1/blueprints/depsolve/<blueprint_names>"""
        resp = self.server.get("/api/v1/blueprints/depsolve/example-glusterfs")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        blueprints = data.get("blueprints")
        self.assertNotEqual(blueprints, None)
    ...
    def test_14_blueprints_depsolve_empty(self):
        """Test /api/v1/blueprints/depsolve/<blueprint_names> on empty blueprint"""
        test_blueprint = {"description": "An empty blueprint",
                       "name":"void",
                       "version": "0.1.0"}
        resp = self.server.post("/api/v1/blueprints/new",
        ...
        resp = self.server.get("/api/v1/blueprints/depsolve/void")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        blueprints = data.get("blueprints")
        self.assertNotEqual(blueprints, None)
    ...
    def test_15_blueprints_freeze(self):
        """Test /api/v1/blueprints/freeze/<blueprint_names>"""
        resp = self.server.get("/api/v1/blueprints/freeze/example-glusterfs")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        blueprints = data.get("blueprints")
        self.assertNotEqual(blueprints, None)
    ...
    def test_projects_list(self):
        """Test /api/v1/projects/list"""
        resp = self.server.get("/api/v1/projects/list")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        projects = data.get("projects")
        self.assertEqual(len(projects) > 10, True)
        ...
        # Make sure limit=0 still returns the correct total
        resp = self.server.get("/api/v1/projects/list?limit=0")
        data = json.loads(resp.data)
        self.assertEqual(data["total"], expected_total)

    def test_projects_info(self):
        """Test /api/v1/projects/info/<project_names>"""
        resp = self.server.get("/api/v1/projects/info/bash")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        projects = data.get("projects")
        self.assertEqual(len(projects) > 0, True)
    ...
    def test_projects_depsolve(self):
        """Test /api/v1/projects/depsolve/<project_names>"""
        resp = self.server.get("/api/v1/projects/depsolve/bash")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        deps = data.get("projects")
        self.assertEqual(len(deps) > 10, True)
    ...
    def test_projects_source_00_list(self):
        """Test /api/v1/projects/source/list"""
        resp = self.server.get("/api/v1/projects/source/list")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        print(data["sources"])
        # Make sure it lists some common sources
    ...
    def test_projects_source_01_delete_system(self):
        """Test /api/v1/projects/source/delete a system source"""
        if self.rawhide:
            resp = self.server.delete("/api/v1/projects/source/delete/rawhide")
        else:
            resp = self.server.delete("/api/v1/projects/source/delete/fedora")
        self.assertEqual(resp.status_code, 400)
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False)
    ...
    def test_projects_source_02_delete_single(self):
        """Test /api/v1/projects/source/delete a single source"""
        resp = self.server.delete("/api/v1/projects/source/delete/single-repo")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data, {"status":True})
    ...
    def test_projects_source_03_delete_unknown(self):
        """Test /api/v1/projects/source/delete an unknown source"""
        resp = self.server.delete("/api/v1/projects/source/delete/unknown-repo")
        self.assertEqual(resp.status_code, 400)
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False)
    ...
    def test_projects_source_04_delete_multi(self):
        """Test /api/v1/projects/source/delete a source from a file with multiple sources"""
        resp = self.server.delete("/api/v1/projects/source/delete/lorax-3")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data, {"status":True})
    ...
    def test_modules_list(self):
        """Test /api/v1/modules/list"""
        resp = self.server.get("/api/v1/modules/list")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        modules = data.get("modules")
        self.assertEqual(len(modules) > 10, True)
        ...
        resp = self.server.get("/api/v1/modules/list/d*")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        modules = data.get("modules")
        self.assertEqual(len(modules) > 0, True)
        ...
        # Make sure limit=0 still returns the correct total
        resp = self.server.get("/api/v1/modules/list?limit=0")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["total"], expected_total)

    def test_modules_info(self):
        """Test /api/v1/modules/info"""
        resp = self.server.get("/api/v1/modules/info/bash")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        modules = data.get("modules")
        self.assertEqual(len(modules) > 0, True)
    ...
    def test_blueprint_new_branch(self):
        """Test the /api/v1/blueprints/new route with a new branch"""
        test_blueprint = {"description": "An example GlusterFS server with samba",
                       "name":"example-glusterfs",
                       "version": "0.2.0",
                       "modules":[GLUSTERFS_GLOB,
        ...
        resp = self.server.post("/api/v1/blueprints/new?branch=test",
                                data=json.dumps(test_blueprint),
                                content_type="application/json")
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})

        resp = self.server.get("/api/v1/blueprints/info/example-glusterfs?branch=test")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        blueprints = data.get("blueprints")
        self.assertEqual(len(blueprints), 1)
    ...
    def test_compose_01_types(self):
        """Test the /api/v1/compose/types route"""
        resp = self.server.get("/api/v1/compose/types")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual({"name": "tar", "enabled": True} in data["types"], True)
    ...
    def test_compose_03_status_fail(self):
        """Test that requesting a status for a bad uuid is empty"""
        resp = self.server.get("/api/v1/compose/status/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["uuids"], [], "Failed to get empty result bad uuid: %s" % data)
    ...
    def test_compose_04_cancel_fail(self):
        """Test that requesting a cancel for a bad uuid fails."""
        resp = self.server.delete("/api/v1/compose/cancel/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False, "Failed to get an error for a bad uuid: %s" % data)
        self.assertEqual(data["errors"], [{"id": UNKNOWN_UUID, "msg": "NO-UUID-TO-SEE-HERE is not a valid build uuid"}],
    ...
    def test_compose_05_delete_fail(self):
        """Test that requesting a delete for a bad uuid fails."""
        resp = self.server.delete("/api/v1/compose/delete/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["errors"], [{"id": UNKNOWN_UUID, "msg": "no-uuid-to-see-here is not a valid build uuid"}],
                         "Failed to get an error for a bad uuid: %s" % data)
    ...
    def test_compose_06_info_fail(self):
        """Test that requesting info for a bad uuid fails."""
        resp = self.server.get("/api/v1/compose/info/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False, "Failed to get an error for a bad uuid: %s" % data)
        self.assertEqual(data["errors"], [{"id": UNKNOWN_UUID, "msg": "NO-UUID-TO-SEE-HERE is not a valid build uuid"}],
    ...
    def test_compose_07_metadata_fail(self):
        """Test that requesting metadata for a bad uuid fails."""
        resp = self.server.get("/api/v1/compose/metadata/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False, "Failed to get an error for a bad uuid: %s" % data)
        self.assertEqual(data["errors"], [{"id": UNKNOWN_UUID, "msg": "NO-UUID-TO-SEE-HERE is not a valid build uuid"}],
    ...
    def test_compose_08_results_fail(self):
        """Test that requesting results for a bad uuid fails."""
        resp = self.server.get("/api/v1/compose/results/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False, "Failed to get an error for a bad uuid: %s" % data)
        self.assertEqual(data["errors"], [{"id": UNKNOWN_UUID, "msg": "NO-UUID-TO-SEE-HERE is not a valid build uuid"}],
    ...
    def test_compose_09_logs_fail(self):
        """Test that requesting logs for a bad uuid fails."""
        resp = self.server.get("/api/v1/compose/logs/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False, "Failed to get an error for a bad uuid: %s" % data)
        self.assertEqual(data["errors"], [{"id": UNKNOWN_UUID, "msg": "NO-UUID-TO-SEE-HERE is not a valid build uuid"}],
    ...
    def test_compose_10_log_fail(self):
        """Test that requesting log for a bad uuid fails."""
        resp = self.server.get("/api/v1/compose/log/NO-UUID-TO-SEE-HERE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], False, "Failed to get an error for a bad uuid: %s" % data)
        self.assertEqual(data["errors"], [{"id": UNKNOWN_UUID, "msg": "NO-UUID-TO-SEE-HERE is not a valid build uuid"}],
    ...
    def test_compose_11_create_failed(self):
        """Test the /api/v1/compose routes with a failed test compose"""
        test_compose = {"blueprint_name": "example-glusterfs",
                        "compose_type": "tar",
                        "branch": "master"}
        ...
        # Wait for it to finish
        self.assertEqual(_wait_for_status(self, build_id, ["FAILED"], api=1), True, "Failed to finish test compose")
        ...
        # Test the /api/v1/compose/status/<uuid> route
        resp = self.server.get("/api/v1/compose/status/%s" % build_id)
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        ids = [(e["id"], e["queue_status"]) for e in data["uuids"]]
        self.assertEqual((build_id, "FAILED") in ids, True, "Failed build not listed by /compose/status")
        ...
        # Wait for it to start
        self.assertEqual(_wait_for_status(self, cancel_id, ["RUNNING"], api=1), True, "Failed to start test compose")

        # Cancel the build
        resp = self.server.delete("/api/v1/compose/cancel/%s" % cancel_id)
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertEqual(data["status"], True, "Failed to cancel test compose: %s" % data)

        # Delete the failed build
        # Test the /api/v1/compose/delete/<uuid> route
        resp = self.server.delete("/api/v1/compose/delete/%s" % build_id)
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        ids = [(e["uuid"], e["status"]) for e in data["uuids"]]
        self.assertEqual((build_id, True) in ids, True, "Failed to delete test compose: %s" % data)
    ...
    def test_compose_12_create_finished(self):
        """Test the /api/v1/compose routes with a finished test compose"""
        test_compose = {"blueprint_name": "example-custom-base",
                        "compose_type": "tar",
                        "branch": "master"}
        ...
        # Test the /api/v1/compose/results/<uuid> route
        resp = self.server.get("/api/v1/compose/results/%s" % build_id)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.data) > 1024, True)

        # Test the /api/v1/compose/image/<uuid> route
        resp = self.server.get("/api/v1/compose/image/%s" % build_id)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.data) > 0, True)
        self.assertEqual(resp.data, b"TEST IMAGE")
    ...
    def test_compose_13_status_filter(self):
        """Test filter arguments on the /api/v1/compose/status route"""
        # Get a couple compose results going so we have something to filter
        test_compose_fail = {"blueprint_name": "example-glusterfs",
                             "compose_type": "tar",
                             "branch": "master"}
        ...
        # Wait for it to finish
        self.assertEqual(_wait_for_status(self, build_id_fail, ["FAILED"], api=1), True, "Failed to finish test compose")
        ...
        # Wait for it to finish
        self.assertEqual(_wait_for_status(self, build_id_success, ["FINISHED"], api=1), True, "Failed to finish test compose")

        # Test that both composes appear in /api/v1/compose/status/*
        resp = self.server.get("/api/v1/compose/status/*")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        ids = [e["id"] for e in data["uuids"]]
        self.assertIn(build_id_success, ids, "Finished build not listed by /compose/status/*")
        ...
        # Filter by status
        resp = self.server.get("/api/v1/compose/status/*?status=FAILED")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        ids = [e["id"] for e in data["uuids"]]
        self.assertIn(build_id_fail, ids, "Failed build not listed by /compose/status status filter")
    ...
    def test_blueprints_list_branch(self):
        resp = self.server.get("/api/v1/blueprints/list?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_info_input(self):
        """Test the blueprints/info input character checking"""
        # /api/v1/blueprints/info/<blueprint_names>
        resp = self.server.get("/api/v1/blueprints/info/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.get("/api/v1/blueprints/info/example-http-server?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.get("/api/v1/blueprints/info/example-http-server?format=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_changes_input(self):
        """Test the blueprints/changes input character checking"""
        # /api/v1/blueprints/changes/<blueprint_names>
        resp = self.server.get("/api/v1/blueprints/changes/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.get("/api/v1/blueprints/changes/example-http-server?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_new_input(self):
        """Test the blueprints/new input character checking"""
        # /api/v1/blueprints/new
        test_blueprint = {"description": "An example GlusterFS server with samba",
                       "name":UTF8_TEST_STRING,
                       "version": "0.2.0",
                       "modules":[GLUSTERFS_GLOB,
        ...
        test_blueprint["name"] = "example-glusterfs"
        resp = self.server.post("/api/v1/blueprints/new?branch=" + UTF8_TEST_STRING,
                                data=json.dumps(test_blueprint),
                                content_type="application/json")
        self.assertInputError(resp)
    ...
    def test_blueprints_delete_input(self):
        """Test the blueprints/delete input character checking"""
        resp = self.server.delete("/api/v1/blueprints/delete/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.delete("/api/v1/blueprints/delete/example-http-server?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)
    ...
    def test_blueprints_workspace_input(self):
        ...
        test_blueprint["name"] = "example-glusterfs"
        resp = self.server.post("/api/v1/blueprints/workspace?branch=" + UTF8_TEST_STRING,
                                data=json.dumps(test_blueprint),
                                content_type="application/json")
        self.assertInputError(resp)
    ...
    def test_blueprints_workspace_delete_input(self):
        """Test the DELETE blueprints/workspace input character checking"""
        resp = self.server.delete("/api/v1/blueprints/workspace/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.delete("/api/v1/blueprints/workspace/example-http-server?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_undo_input(self):
        """Test the blueprints/undo/... input character checking"""
        resp = self.server.post("/api/v1/blueprints/undo/" + UTF8_TEST_STRING + "/deadbeef")
        self.assertInputError(resp)

        resp = self.server.post("/api/v1/blueprints/undo/example-http-server/deadbeef?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_tag_input(self):
        """Test the blueprints/tag input character checking"""
        resp = self.server.post("/api/v1/blueprints/tag/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.post("/api/v1/blueprints/tag/example-http-server?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_diff_input(self):
        """Test the blueprints/diff input character checking"""
        # /api/v1/blueprints/diff/<blueprint_name>/<from_commit>/<to_commit>
        resp = self.server.get("/api/v1/blueprints/diff/" + UTF8_TEST_STRING + "/NEWEST/WORKSPACE")
        self.assertInputError(resp)

        resp = self.server.get("/api/v1/blueprints/diff/example-http-server/NEWEST/WORKSPACE?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_freeze_input(self):
        """Test the blueprints/freeze input character checking"""
        resp = self.server.get("/api/v1/blueprints/freeze/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.get("/api/v1/blueprints/freeze/example-http-server?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.get("/api/v1/blueprints/freeze/example-http-server?format=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_blueprints_depsolve_input(self):
        """Test the blueprints/depsolve input character checking"""
        resp = self.server.get("/api/v1/blueprints/depsolve/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

        resp = self.server.get("/api/v1/blueprints/depsolve/example-http-server?branch=" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_projects_info_input(self):
        """Test the projects/info input character checking"""
        resp = self.server.get("/api/v1/projects/info/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_projects_depsolve_input(self):
        """Test the projects/depsolve input character checking"""
        resp = self.server.get("/api/v1/projects/depsolve/" + UTF8_TEST_STRING)
        self.assertInputError(resp)
    ...
    def test_projects_source_delete_input(self):
        """Test the projects/source/delete input character checking"""
        resp = self.server.delete("/api/v1/projects/source/delete/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_modules_list_input(self):
        """Test the modules/list input character checking"""
        resp = self.server.get("/api/v1/modules/list/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_modules_info_input(self):
        """Test the modules/info input character checking"""
        resp = self.server.get("/api/v1/modules/info/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_status_input(self):
        """Test the compose/status input character checking"""
        resp = self.server.get("/api/v1/compose/status/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_cancel_input(self):
        """Test the compose/cancel input character checking"""
        resp = self.server.delete("/api/v1/compose/cancel/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_delete_input(self):
        """Test the compose/delete input character checking"""
        resp = self.server.delete("/api/v1/compose/delete/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_info_input(self):
        """Test the compose/info input character checking"""
        resp = self.server.get("/api/v1/compose/info/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_metadata_input(self):
        """Test the compose/metadata input character checking"""
        resp = self.server.get("/api/v1/compose/metadata/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_results_input(self):
        """Test the compose/results input character checking"""
        resp = self.server.get("/api/v1/compose/results/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_logs_input(self):
        """Test the compose/logs input character checking"""
        resp = self.server.get("/api/v1/compose/logs/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_image_input(self):
        """Test the compose/image input character checking"""
        resp = self.server.get("/api/v1/compose/image/" + UTF8_TEST_STRING)
        self.assertInputError(resp)

    def test_compose_log_input(self):
        """Test the compose/log input character checking"""
        resp = self.server.get("/api/v1/compose/log/" + UTF8_TEST_STRING)
        self.assertInputError(resp)
    ...
    def test_deleted_bp_00_setup(self):
        ...
        resp = self.server.delete("/api/v1/blueprints/delete/deleted-blueprint")
        data = json.loads(resp.data)
        self.assertEqual(data, {"status":True})

    def test_deleted_bp_01_show(self):
        """Test blueprint show with deleted blueprint"""
        resp = self.server.get("/api/v1/blueprints/info/deleted-blueprint")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertTrue(len(data["errors"]) > 0)
        self.assertEqual(data["errors"][0]["id"], "UnknownBlueprint")
    ...
    def test_deleted_bp_02_depsolve(self):
        """Test blueprint depsolve with deleted blueprint"""
        resp = self.server.get("/api/v1/blueprints/depsolve/deleted-blueprint")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertTrue(len(data["errors"]) > 0)
        self.assertEqual(data["errors"][0]["id"], "UnknownBlueprint")
    ...
    def test_deleted_bp_03_diff(self):
        """Test blueprint diff with deleted blueprint"""
        resp = self.server.get("/api/v1/blueprints/diff/deleted-blueprint/NEWEST/WORKSPACE")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertTrue(len(data["errors"]) > 0)
        self.assertEqual(data["status"], False)
    ...
    def test_deleted_bp_04_freeze(self):
        """Test blueprint freeze with deleted blueprint"""
        resp = self.server.get("/api/v1/blueprints/freeze/deleted-blueprint")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertTrue(len(data["errors"]) > 0)
        self.assertEqual(data["errors"][0]["id"], "UnknownBlueprint")
    ...
    def test_deleted_bp_05_tag(self):
        """Test blueprint tag with deleted blueprint"""
        resp = self.server.post("/api/v1/blueprints/tag/deleted-blueprint")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        self.assertTrue(len(data["errors"]) > 0)
        self.assertEqual(data["errors"][0]["id"], "UnknownBlueprint")
...
class RepoCacheAPIV0TestCase(unittest.TestCase):
...
class RepoCacheAPIV1TestCase(unittest.TestCase):
    """Test to make sure that changes to the repository are picked up immediately."""
    @classmethod
    def setUpClass(self):
        repo_dir = tempfile.mkdtemp(prefix="lorax.test.repo.")
    ...
    def add_new_source(self, repo_dir):
        json_source = """{"id": "new-repo-1", "name": "New repo 1", "url": "file:///tmp/lorax-test-repo/",
                          "type": "yum-baseurl", "check_ssl": false, "check_gpg": false}"""
        self.assertTrue(len(json_source) > 0)
        resp = self.server.post("/api/v1/projects/source/new",
                                data=json_source,
                                content_type="application/json")
    ...
    def test_metadata_expires(self):
        ...
        # Depsolve the blueprint
        resp = self.server.get("/api/v1/blueprints/depsolve/milhouse-test")
        data = json.loads(resp.data)
        self.assertNotEqual(data, None)
        blueprints = data.get("blueprints")
        self.assertNotEqual(blueprints, None)
