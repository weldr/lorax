...
boot_image() {
    QEMU_BOOT=$1
    TIMEOUT=$2
    rlRun -t -c "$QEMU -m 1024 $QEMU_BOOT -cpu host -nographic -monitor none \
...
verify_image() {
    SSH_USER="$1"
    SSH_MACHINE="$2"
    SSH_OPTS="-vvv -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o BatchMode=yes $3"
