...
def generate_module_info(moddir, outfile=None):
    ...
    def read_module_set(name):
        return set(
            l.strip() for l in open(joinpaths(moddir, name)) if ".ko" in l)
...
class RuntimeBuilder(object):
    ...
    def _install_branding(self):
        ...
        if self.vars.product.variant:
            variant = [p for p in pkgs if
                       p.endswith("-" + self.vars.product.variant.lower())]
            if variant:
                release = variant[0]
        if not release:
            release = pkgs[0]
    ...
    def verify(self):
        ...
        for path in (str(x) for x in
                     itertools.chain(usr_bin.iterdir(), usr_sbin.iterdir()) \
                     if x.is_file()):
        ...
        filename = ''
        for line in execWithCapture(
                'ldd', elf_files, root=self.vars.root, log_output=False,
                filter_stderr=True).split('\n'):
            if line and not line[0].isspace():
                # New filename header, strip the : at the end and save
                filename = line[:-1]
            elif 'not found' in line:
                logger.error('%s, needed by %s, not found', line.split()[0],
                             filename)
                status = False
    ...
    def create_squashfs_runtime(self, outfile="/var/tmp/squashfs.img",
                                compression="xz", compressargs=None, size=2):
    ...
    def create_ext4_runtime(self, outfile="/var/tmp/squashfs.img",
                            compression="xz", compressargs=None, size=2):
...
class TreeBuilder(object):
    ...
    def __init__(self, product, arch, inroot, outroot, runtime, isolabel,
                 domacboot=True, doupgrade=True,
                 templatedir=None, add_templates=None, add_template_vars=None,
                 workdir=None, extra_boot_args=""):
