...
class VirtMachineTestCase(unittest.TestCase):
    ...
    def setUpTestMachine(self, image, identity_file=None):
        ...
        if identity_file:
            self.machine = testvm.VirtMachine(image, networking=self.network.host(), cpus=2, memory_mb=3072, identity_file=identity_file)
        else:
            self.machine = testvm.VirtMachine(image, networking=self.network.host(), cpus=2, memory_mb=3072)
